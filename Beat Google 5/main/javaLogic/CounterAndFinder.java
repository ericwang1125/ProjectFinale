...
public class CounterAndFinder{
	...
	public ArrayList<Page> FinderBoyerMoore(String T){
        ...
        while(a < T.length()) {
        	if(T.charAt(a) == P.charAt(b)) {
        		if(b == 0) {
        			a++;
                    int a2 = a;
                    while(T.charAt(a) != (char)34){ //
                        a++;
                    }
                    a++;
                    String href = T.substring(a2, a);
                    retVal.add(new Page(href));
        		}else {
        			a--;
        			b--;
        		}
    ...
    public ArrayList<Page> findSubWeb() throws IOException{
		if (content == null){
		    content = fetchContent();
		}
		...
		return FinderBoyerMoore(content);
    }
}
